...
import { ClusterScatter, MultiBarChart } from "@/components/charts";
...
const CLUSTER_COLORS = ["#dc2626", "#2563eb", "#059669", "#d97706", "#7c3aed", "#0891b2", "#db2777", "#65a30d"];
...
export default function ClusteringPage() {
  ...
  return (
    <PageShell
      title={t(filters.language, "page_clustering_title")}
      subtitle={fr ? "Segmentation comportementale des sites (KMeans) · projection PCA 2D · score de santé" : "Behavioural site segmentation (KMeans) · 2D PCA projection · health score"}
    >
      {!hasDates ? (
                ...
                {[2, 3, 4, 5, 6].map((value) => (
                  <button
                    ...
                    className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${
                      clusterCount === value ? "border-red-500 bg-red-600 text-white" : "border-slate-300 bg-white text-slate-600"
                    }`}
                  >
                    {value}
                  </button>
