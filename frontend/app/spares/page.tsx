...
export default function SparesPage() {
  ...
  const displayRows = useMemo(
    () =>
      data.rows.map((row) => ({
        ...
        [fr ? "Base installée" : "Installed base"]: row.installed_base,
        Sites: row.sites,
        [fr ? "Remplacements" : "Replacements"]: row.replacements_period,
        [fr ? "Taux annuel %" : "Annual rate %"]: row.annual_failure_rate_pct,
        [fr ? "Demande attendue" : "Expected demand"]: row.expected_demand,
        [fr ? "Spares recommandés" : "Recommended spares"]: row.recommended_spares,
        [fr ? "Criticité" : "Criticality"]: row.criticality,
      })),
    [data.rows, fr],
  );
  ...
  return (
    <PageShell
      title={t(filters.language, "page_spares_title")}
      subtitle={fr ? "Besoin en pièces de rechange par code produit · churn observé → demande projetée + stock de sécurité (Poisson)" : "Spare parts need per product code · observed churn → projected demand + Poisson safety stock"}
    >
      {!hasDates ? (
        ...
        <div className="space-y-5">
                ...
                {[30, 90, 180, 365].map((value) => (
                  <button
                    ...
                    className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${
                      horizon === value ? "border-red-500 bg-red-600 text-white" : "border-slate-300 bg-white text-slate-600"
                    }`}
                  >
                    {value}j
                  </button>
          ...
          {data.top_chart.length ? (
            <section>
              ...
              <MultiBarChart
                ...
                xKey="product_code"
                bars={[
                  { key: "recommended_spares", color: "#dc2626" },
              ...
              <p className="mt-2 text-xs text-slate-500">
                {fr
                  ? "Barre rouge: spares recommandés sur l’horizon ; barre grise: remplacements observés sur la période."
                  : "Red bar: recommended spares over the horizon; grey bar: replacements observed over the period."}
              </p>
            </section>
          ) : loading ? (
          ...
          <section className="premium-card rounded-2xl p-4">
            ...
            <div className="mb-3 grid grid-cols-2 gap-2 lg:grid-cols-4">
              <div className="rounded-lg border border-red-100 bg-white p-2 text-xs">
