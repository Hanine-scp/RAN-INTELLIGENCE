...
export default function AnomaliesPage() {
  ...
  const kpis = useMemo(
    () => [
      { label: fr ? "Alertes totales" : "Total alerts", value: String(data.summary.total) },
      { label: "Critical", value: String(data.summary.critical) },
      { label: "High", value: String(data.summary.high) },
      { label: "Medium", value: String(data.summary.medium) },
      { label: fr ? "Sites impactés" : "Sites impacted", value: String(data.summary.sites_impacted) },
    ],
    [data.summary, fr],
  );
    ...
    return data.rows
      .map((row) => {
        ...
        return {
          ...
          [fr ? "Nom" : "Name"]: String(row.site_name ?? ""),
          [fr ? "Type" : "Type"]: String(row.anomaly_type ?? ""),
          [fr ? "Détail" : "Detail"]: String(row.detail ?? ""),
          [fr ? "Sévérité" : "Severity"]: Number(row.severity_score ?? 0),
          [fr ? "Niveau" : "Level"]: String(row.level ?? ""),
          [fr ? "Source données" : "Data source"]: String(row.evidence ?? ""),
          [fr ? "Statut" : "Status"]: isAck ? (fr ? "Acquittée" : "Acknowledged") : (fr ? "Active" : "Active"),
        } as AnomalyRow;
      })
      .filter((row) => (hideAck ? !acknowledged.includes(String(row.alert_id)) : true));
  }, [data.rows, acknowledged, hideAck, fr]);
  ...
  return (
    <PageShell
      title={t(filters.language, "page_anomalies_title")}
      subtitle={fr ? "Moteur de règles métier · scoring de sévérité · acquittement & validation expert" : "Business rule engine · severity scoring · acknowledge & expert validation"}
    >
      {!hasDates ? (
