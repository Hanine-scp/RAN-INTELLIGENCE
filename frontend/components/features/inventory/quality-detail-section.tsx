...
import { useQuality } from "@/lib/hooks/use-quality";
...
export function QualityDetailSection() {
  ...
  const { data, error, isLoading } = useQuality(payload);
  ...
  const onInvestigateSite = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setInvestigationError(error instanceof Error ? error.message : "Site investigation failed.");
    }
  };
  ...
  const onInvestigateSerial = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSerialRows([]);
      setInvestigationError(error instanceof Error ? error.message : "Serial investigation failed.");
    }
  };
  ...
  return (
    <div className="space-y-4">
      {(errorMessage || investigationError) ? (
        <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">{errorMessage || investigationError}</p>
      ) : null}
