...
import { CellTechnologyShareCard } from "@/components/ui/cell-technology-share-card";
...
import { getSiteKpiTimeseries, investigateSite, type SiteKpiTimeseries } from "@/lib/api";
...
export function SitesTableSection({ payload, language }: SitesTableSectionProps) {
  ...
  const { data, error, isLoading } = useSitesPage(payload, {
    page,
    page_size: DEFAULT_TABLE_PAGE_SIZE,
    search: payload.site_search ?? "",
  });
  const rows = data?.rows ?? [];
  const totalCount = Number(data?.total_count ?? rows.length);
  ...
  }, [payload.effective_dates.length, payload.selected_dates.length]);
  ...
  useEffect(() => {
    ...
    setSelectedSiteId((current) =>
      current && !rows.some((row) => String(row.site_id ?? "") === current) ? null : current,
    );
  }, [rows]);
